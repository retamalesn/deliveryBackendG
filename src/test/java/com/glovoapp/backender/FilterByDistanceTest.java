...
class FilterByDistanceTest {
    ...
    @Test
    public void filterByDistanceInMotorcycleOrElectricalTest() {
    	...
    	configuration.setCourierLimitDistance(1D);
    	
    	FilterByDistance filterByDistance = new FilterByDistance();
        assertEquals(orderListExpected,filterByDistance.filter(orderList, courier, configuration));
    }
    
    @Test
    public void filterByDistanceInBicycleTest() {
    	...
    	List<Order> orderList = Arrays.asList(order1, order2, order3);
    	List<Order> orderListExpected = Arrays.asList();
